...
jQuery(document).ready(function() {
	...
	jQuery(".action").click(function(){
		...
		var edt = jQuery("input[name=\"edt\"]").val();
		if(jQuery(this).val() == "edit"){
			jQuery("input[name=\"task\"]").val(edt+"."+jQuery(this).val());
		}else{
			jQuery("input[name=\"task\"]").val(sub+"."+jQuery(this).val());
		}
...
?>
<form action="<?php echo JRoute::_('index.php?option=com_weldusers&view=users');?>" method="post" name="adminForm" id="adminForm">

	<div class="control-group">
		<div class="controls">
			<a href="<?php echo JRoute::_('index.php?option=com_weldusers&view=user&layout=edit', false);?>"><button type="button" class=" btn btn-small btn-success"><span class="icon-new icon-white"></span>新建</button></a>
			<button type="button" value="edit" class="action btn btn-small btn-info"><span class="icon-edit"></span>编辑</button>
			<button type="button" value="activate" class="action btn btn-small btn-inverse"><span class="icon-publish"></span>激活</button>
			<button type="button" value="block" class="action btn btn-small btn-warning"><span class="icon-lock"></span>锁定</button>
			<button type="button" value="changeBlock" class="action btn btn-small "><span class="icon-unlock"></span>解锁</button>
			<button type="button" value="delete" class="action btn btn-small btn-danger"><span class="icon-unpublish"></span>删除</button>
			<button type="button" value="batch" class="action btn btn-small btn-primary"><span class="icon-publish"></span>批量处理</button>
		...
		<?php else : ?>
			<table class="table table-striped" id="userList">
				...
				<?php foreach ($this->items as $i => $item) :
					...
					$canChange = $loggeduser->authorise('core.edit.state',	'com_weldusers');
				...
				?>
					<tr class="row<?php echo $i % 2; ?>">
						...
						<td>
							<div class="name">
							<?php if ($canEdit) : ?>
								<a href="<?php echo JRoute::_('index.php?option=com_weldusers&task=user.edit&id=' . (int) $item->id); ?>" title="<?php echo JText::sprintf('COM_USERS_EDIT_USER', $this->escape($item->name)); ?>">
							...
							<?php endif; ?>
							<?php if (JDEBUG) : ?>
								<div class="small"><a href="<?php echo JRoute::_('index.php?option=com_weldusers&view=debuguser&user_id=' . (int) $item->id);?>">
			...
			<?php if ($loggeduser->authorise('core.create', 'com_weldusers')
				&& $loggeduser->authorise('core.edit', 'com_weldusers')
				&& $loggeduser->authorise('core.edit.state', 'com_weldusers')) : ?>
				<?php echo JHtml::_(
		...
		<input type="hidden" name="edt" value="user">
