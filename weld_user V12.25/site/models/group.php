 ...
 * @since  1.6
 */
class WeldUsersModelGroup extends JModelAdmin
{
	/**
	 * Constructor
	 *
...
//		return parent::save($data);
	...
	public function getGroupLevel(){
		...
		foreach ($levels as $values){
			$rules[$values['id']] = json_decode($values['rules']);
		}
		return $rules;
	}
//*******************向levels表插入数据
	public function setRules($id,$data){
		$levelRule = array();
		$db = JFactory::getDbo();
		$query = $db->getQuery(TRUE);
		$query
			->select('id,rules')
			->from($db->quoteName('#__viewlevels'))
			->where($db->quoteName('id').'IN ('.implode(',',$data).')');
		$db->setQuery($query);
		$rules = $db->loadAssocList();
		$query->clear();
		foreach ($rules as $key => $value){
			$rule = json_decode($value['rules']);
			if(!in_array($id,$rule)){
				array_push($rule,$id);
			}

			$levelRule[$value['id']] = json_encode($rule);

		}
//		var_dump($levelRule);die;
		$sql = ' UPDATE `#__viewlevels` SET `rules` = CASE `id` ';
		foreach($levelRule as $key => $value){
			$sql .= ' WHEN '.$key.' THEN \''.$value.'\'';
		}
		$sql = $sql.' END';
//		var_dump($sql,$id,$data);die;
		$query->setQuery($sql);
