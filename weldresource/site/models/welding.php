 ...
 */
class WeldResourceModelWelding extends JModelForm{
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'Welding', $prefix = 'WeldResourceTable', $config = array())
	{
	...
	public function loadFormData()
	{
		$dada = JFactory::getApplication()->getUserState(
			'com_weldresource.edit.welding.data',
			array()
		);
		if (empty($dada)){
			$dada = $this->getItem();
	...
	public function delete(&$pks){
		try{
			...
			$query->delete($db->quoteName('#__weldresource_welding'))
				->where($conditions);
			$db->setQuery($query);
			$db->execute();
			$db->transactionCommit();
