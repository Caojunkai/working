 ...
 * @since  1.6
 */
class WeldResourceModelWeldings extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$categoryId = $this->getUserStateFromRequest($this->context . '.filter.brand', 'filter_brand');
		$this->setState('filter.brand', $categoryId);
	...
	protected function getListQuery()
	{
		...
		$query->select(
			$this->getState('list.select',
				'a.id, a.name, a.num, a.brand, a.diameter, a.density'
			)
		);
		$query->from('#__weldresource_welding AS a');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.diameter LIKE ' . $search . ')');
			}
		}
